# ============================================================
# Advanced Educational YouTube Downloader & Professional Compressor
# ============================================================
#
# ======================= الميزات ============================
#
...
# ✔ دعم Playlist
# ✔ اختيار جودة مع عرض الحجم المتوقع لكل جودة
# ✔ MP3 16kbps
...
# ✔ شريط تقدم دقيق للضغط مع تقدير الوقت المتبقي
#
# ميزات الواجهة:
# ✔ دعم Drag & Drop
...
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Educational YouTube Downloader & Compressor")
    ...
    def fetch_info(self):
        ...
        self.log_box.insert("end", f"Found {len(labels)} formats.\n")
        ...
        for line in process.stderr:
            if "time=" in line and self.duration:
                ...
                try:
                    ...
                    percent = min(current/self.duration,1)
                    self.progress.set(percent)
                    elapsed = time.time()-start_time
                    remaining = (elapsed/percent)*(1-percent) if percent>0 else 0
                    self.log_box.insert("end", f"\rProgress: {percent*100:.1f}% | ETA: {int(remaining)}s")
